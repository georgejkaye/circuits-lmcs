...
the diagrams, as illustrated in \cref{fig:stmc-axioms}.
...
\begin{figure*}
    ...
    =
    \iltikzfig{strings/traced/vanishing-rhs}[box=f,colour=white,dom=m,cod=n,trace1=x,trace2=y]
    \quad
    ...
    \)
    \caption{
        The equations of traced PROPS in
...
\begin{nota}\label{not:arbitrary-width-structure}
    ...
    Structural generators for \(0\)-bit wires are drawn as empty space or as
...
To answer this question, we will consider \emph{Mealy machines}.
...
However, they can also be viewed in a more mathematical way as a
\emph{coalgebra}.
...
\begin{cor}[Corollary 2.3, \cite{rutten2006algebraic}]\label{cor:minimal-mealy}
...
\subsection{Between circuits and Mealy machines}\label{sec:synthesis}
...
We have now finally established the \emph{denot} of a sequential circuit \(
\iltikzfig{strings/category/f}[box=f,colour=seq,dom=m,cod=n]
...
\begin{exa}
    ...
    \autoref{sec:denotational-belnap}.
...
\subsection{Denotational semantics for Belnap logic}\label{sec:denotational-belnap}
...
for the Belnap interpretation introduced in \autoref{ex:belnap-interpretation}.
...
\begin{lem}\label{lem:boolean-complete}
...
\begin{lem}\label{lem:belnap-bool-correspondence}
...
\begin{lem}
    ...
    A monotone functions \(\booleans^m \to \booleans\) can be expressed with
    the operations \(\{\land,\lor,1\}\).
\end{lem}
\begin{proof}
    This progresses as with \autoref{lem:boolean-complete}, but if the element of
...
\begin{cor}\label{cor:func-complete-falsy-truthy}
    A mnonotone \((\values_0)^m \to \values_0\) can be expressed
    with the operations \(\{\land,\lor,\belnapfalse\}\), and a monotone
    function \((\values_1)^m \to \values_1\) can be expressed with the
    operations \(\{\land,\lor,\belnaptrue\}\).
\end{cor}
\begin{proof}
    There is an order isomorphism
    \(\values_0 \cong \values_1 \cong \booleans\) so any monotone function in
    the Belnap subsets can be viewed as a monotone Boolean function.
    This means the strategy of \autoref{lem:boolean-complete-monotone} can
    be applied using \autoref{lem:belnap-bool-correspondence}.
\end{proof}
...
\begin{lem}
    ...
    The functions in \autoref{def:translation-tables} can be expressed using
    \(\{\land,\lor,\neg,\bot\}\).
\end{lem}
\begin{proof}
...
\begin{defi}\label{def:falsy-truthy-functions}
...
\begin{proof}
    ...
    \right).
    \).
    By \autoref{cor:func-complete-falsy-truthy}, \(f_0\) and \(f_1\) can be defined
    using \(\{\land,\lor,\belnaptrue,\belnapfalse\}\), and by
    \autoref{lem:translation-functions}, \(\psi^0_0,\psi^1_0,\psi^0_1\) and
...
\begin{exa}
    ...
    \begin{center}
        ...
        \end{tabular}
    \end{center}
    Using \autoref{cor:func-complete-falsy-truthy}, the corresponding Belnap
    expressions are \[
...
\end{cor}
\begin{proof}
    By repeating the process in \autoref{thm:belnap-complete} for each output.
\end{proof}
...
\subsection{Algebraic semantics for Belnap logic}\label{sec:algebraic-belnap}
...
\autoref{sec:denotational-belnap} that the canonical form for Belnap circuits is a
...
\begin{defi}[Explosion equations]
    Let the set of \emph{explosion equations} \(\mathcal{X}\) be defined as the
    equations listed
    in \autoref{fig:explosion-equations-1} and
    \autoref{fig:explosion-equations-2}.
\end{defi}
...
\begin{lem}\label{lem:explode-copy}
...
\begin{prop}\label{prop:exploded-belnap}
...
\begin{proof}
    ...
    exploded `skeleton', followed by using \autoref{lem:explode-copy} to copy the
    ...
    Using \autoref{lem:explode-copy}, the first circuit can be propagated across
...
\begin{defi}[Normal form equations]
    Let the set of \emph{normal form equations} \(\mathcal{F}\) be defined as
    the equations listed in \autoref{fig:normal-form-equations}.
\end{defi}
...
\begin{lem}\label{lem:truthy-conjunction-normalising}
...
\begin{lem}\label{lem:falsy-conjunction-normalising}
    Given a Belnap circuit \(
    \iltikzfig{circuits/synthesis/normalised-function}[box=f,dom=m,cod=n,values=\belnaptrue]
    \) containing no \(
    \iltikzfig{strings/structure/monoid/merge}[colour=comb]
...
\end{lem}
\begin{proof}
    As \autoref{lem:truthy-conjunction-normalising}, but with the equations
...
\begin{prop}\label{prop:disjunctive-normal-form}
    Given a Belnap circuit \[
        \iltikzfig{circuits/axioms/belnap/translation/exploded-form}[box=f,dom=m,cod=1]
    \] in which \(
...
\begin{proof}
    ...
    This is remedied by applying \autoref{lem:truthy-conjunction-normalising} and
    \autoref{lem:falsy-conjunction-normalising} to the left half of each circuit.
\end{proof}
...
\end{thm}
\begin{proof}
    This follows by applying \autoref{prop:exploded-belnap} followed by
    \autoref{prop:disjunctive-normal-form}.
\end{proof}
...
\subsection{Tidying up}\label{sec:tidy}
...
\begin{defi}[Tidying rules]
    Let the \emph{tidying rules} be defined as in \autoref{fig:tidy-rules}.
\end{defi}
...
\begin{prop}
    Applying the tidying rules to a Mealy form is confluent and
    terminating.
\end{prop}
\begin{proof}
...
A summary of the categories involved is shown in \autoref{fig:map}.
...
cyclic lambda calculi~\cite{hasegawa1997recursion}, fixpoint
...
\(\iltikzfig{strings/traced/trace-from-product-bialg}\) in our \emph{Cartesian}
setting.
...
Sring diagrams are not efficient to work with computationally.
...
circuits based on this work, which can be found at
\url{https://github.com/georgejkaye/circuit-cj}.
