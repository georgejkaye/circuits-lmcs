...
\begin{nota}\label{not:arbitrary-width-structure}
    ...
    (\autoref{not:arbitrary-width-wires}), we can also draw these `thicker'
...
\begin{rem}
    As mentioned during \autoref{not:arbitrary-width-wires}, wires of zero width
...
\begin{proof}
    ...
    be applied using \autoref{lem:belnap-bool-correspondence} to substitute the
...
\begin{lem}
    ...
    The functions in \autoref{def:translation-tables} can be expressed using the
    operations \(\{\land,\lor,\neg,\bot\}\).
\end{lem}
\begin{proof}
...
Our goal for this section is to develop a sound and complete notion of
...
\begin{lem}\label{lem:number-of-states}
    ...
    Then applying \autoref{cor:productivity} successively to a Mealy form of this
