...
\begin{not}\label{not:minimal-mealy-sets}
    ...
    Then, for a state \(u \in \hat{S}\), let \(S_u \subseteq S\) be the set of states
    such that for all \(s \in S_u\),
...
\begin{lem}
    Let \((S, f)\) and \((T, g)\) be Mealy coalgebra.
    Then a function \(\morph{h}{S}{T}\) is a valid Mealy homomorphism if and
    only if there exists a Mealy coalgebra \((U, k)\) such that
    \(
    \streamtomealy{}[\mealytostream{}[(S, f)]]
    =
    \streamtomealy{}[\mealytostream{}[(T, g)]]
    = (U, k)
    \), and for all \(u \in U\) and \(s \in S_u\), \(h(s) \in T_u\).
\end{lem}
\begin{proof}
...
\begin{lem}
    ...
    if \(u_0 \leq u_1\) then \(s_0 \leq s_1\).
\end{lem}
\begin{proof}
    Assume that \(s_1 \leq s_0\).
